...
지하 터널은 총 7 종류의 터널 구조물(코드의 pipe와 같음)로 구성되어 있다.
...
for tc in range(1, 1+int(input())):
    ...
    while Q:
        ...
        if visited[i][j] >= L:
            break
        for d in pipe[tunnel[i][j]]:
            ni = i + d[0]
            nj = j + d[1]
            if 0 <= ni < N and 0 <= nj < M and tunnel[ni][nj] and not visited[ni][nj]:
                for t in pipe[tunnel[ni][nj]]:
                    ti = ni + t[0]
                    tj = nj + t[1]
                    if (ti, tj) == (i, j):
                        break

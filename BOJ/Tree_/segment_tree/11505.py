...
def prefix_multiple(node, st, ed, value_st, value_ed):
    ...
    mid = (st + ed) // 2
    return prefix_multiple(node*2, st, mid, value_st, value_ed) * prefix_multiple(node*2+1, mid+1, ed, value_st, value_ed) % 1000000007
...
for _ in range(M+K):
    ...
    if a == 1:
        ...
        print(prefix_multiple(1, 0, N-1, b-1, c-1))